...
def test_rnn_baseline_returns_losses_and_predictions() -> None:
    torch = pytest.importorskip("torch")

    from ai_orchestra import DEFAULTS, PipelineDefaults
    from ai_orchestra.rnn_baseline import run_rnn_baseline
...
def test_rnn_baseline_rejects_invalid_epoch_count() -> None:
    pytest.importorskip("torch")

    from ai_orchestra.rnn_baseline import run_rnn_baseline

    try:
        run_rnn_baseline(epochs=0)
...
def test_rnn_baseline_rejects_invalid_prediction_count() -> None:
    pytest.importorskip("torch")

    from ai_orchestra.rnn_baseline import run_rnn_baseline

    try:
        run_rnn_baseline(prediction_count=0)
    except ValueError as error:
        assert "positive" in str(error)
    else:
